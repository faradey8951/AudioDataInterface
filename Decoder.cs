            ...
            while (true)
            {
                ...
                if (sync == true) //Если синхронизация обнаружена
                {
                    ...
                    if (dataBlockBuff.Count == 39) //Если блок данных обнаружен
                    {
                        ...
                        Thread.Sleep(10);
                    if (channelSwitch == 0) lock (buff_signalAmplitudesL) buff_signalAmplitudesL.RemoveRange(0, tempSyncIndexes[1] - 1); //Удалить отработанные амплитуды
                    else lock (buff_signalAmplitudesR) buff_signalAmplitudesR.RemoveRange(0, tempSyncIndexes[1] - 1); //Удалить отработанные амплитуды
                }
                else
                {
                    if (channelSwitch == 0)
